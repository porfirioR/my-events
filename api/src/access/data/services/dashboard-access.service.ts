...
export class DashboardAccessService extends BaseAccessService implements IDashboardAccessService {
  ...
  public getSummary = async (userId: number): Promise<DashboardAccessModel> => {
    const [collaborators, transactions, savingsGoals, travels, loans] = await Promise.all([
    ...
    return new DashboardAccessModel(collaborators, transactions, savingsGoals, travels, loans);
  };
  ...
  private getTransactionStats = async (userId: number) => {
    const [total, unsettled, settled] = await Promise.all([
    ...
    return { total: total.count ?? 0, unsettled: unsettled.count ?? 0, settled: settled.count ?? 0 };
  };
  ...
  private getTravelStats = async (userId: number) => {
    ...
    return { total: total.count ?? 0, active: active.count ?? 0, finalized: finalized.count ?? 0 };
  };
