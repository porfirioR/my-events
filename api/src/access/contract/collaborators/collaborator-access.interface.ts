...
} from '../../../access/contract/collaborators';
...
export interface ICollaboratorAccessService {
   ...
   */
  createCollaborator(
    accessRequest: CreateCollaboratorAccessRequest
   ...
   */
  updateCollaborator(
    accessRequest: UpdateCollaboratorAccessRequest
  ): Promise<CollaboratorAccessModel>;
  ...
  getCollaboratorStats(
    userId: number
  ): Promise<{ total: number; internal: number; external: number }>;
  ...
  /**
   * Asigna email a un colaborador existente
  ...
  /**
   * Obtiene el colaborador interno por userId y email del usuario
   * @param userId - ID del usuario propietario
   * @param userEmail - Email del usuario (colaborador interno tiene el mismo email)
   * @returns Promise con el colaborador interno encontrado o null si no existe
   */
  getInternalCollaboratorByUserIdAndEmail(userId: number, userEmail: string): Promise<CollaboratorAccessModel | null>;

  /**
   * ✅ NUEVO: Verifica si existe colaborador interno para el usuario
   * @param userId - ID del usuario propietario
   * @param userEmail - Email del usuario
   * @returns Promise con true si existe, false si no
   */
  hasInternalCollaborator(userId: number, userEmail: string): Promise<boolean>;

  /**
   * ✅ NUEVO: Crea colaborador interno automáticamente
   * @param userId - ID del usuario propietario
   * @param userEmail - Email del usuario (será el email del colaborador interno)
   * @param name - Nombre del colaborador interno
   * @param surname - Apellido del colaborador interno
   * @returns Promise con el colaborador interno creado
   * @throws Error si ya existe o hay problemas en la creación
   */
  createInternalCollaborator(
    userId: number,
    userEmail: string, 
    name: string,
    surname: string
